...
def main():
    ...
    hypes['postprocess'].update({"noise_setting": noise_setting})
    
    use_random = False
    hypes.update({'late_choose_method':use_random})
        ...
        with torch.no_grad():
            ...
            if opt.sample_method == 'unc' and opt.is_hybrid:
                ...
                if non_ego_boxes!=None:
                    temp_unc=non_ego_unc[0]**2+non_ego_unc[1]**2
                    #deliver_index=(non_ego_score>0.3)
                    deliver_index=(temp_unc>=0)
                    if deliver_index is None:
                        continue
                    non_ego_boxes=non_ego_boxes[deliver_index,:,:]
                    non_ego_score=non_ego_score[deliver_index]
                    non_ego_unc=non_ego_unc[:,deliver_index]
                    final_result.append(non_ego_boxes)
                    final_score.append(non_ego_score)
                    non_ego_unc=non_ego_unc[0]**2+non_ego_unc[1]**2
                    final_unc.append(non_ego_unc)
                    points_num=points_num+non_ego_boxes.shape[0]*8
                if (hypes['pose_err']!=0 and non_ego_score!=None and opt.pose_index!=1) :
                    non_ego_score*=opt.pose_index
            ...
            if opt.store_boxes and opt.fusion_method=='late':
                ...
                # pred_box_tensor = non_ego_boxes
                # pred_score = non_ego_score
            if opt.store_boxes:
