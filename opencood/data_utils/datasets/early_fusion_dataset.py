    ...
    class EarlyFusionDataset(cls):
        ...
        def __init__(self, params, visualize, train=True):
            ...
            if self.sample_method == 'unc':
                ...
                self.detect_result=np.load(os.path.join(model_dir, 'val_boxes_down_new_no_error.npy'),allow_pickle=True).item()
                self.sampling_rate=params['sampling_rate']
        ...
        def unc_filtering(self,inputlidar,tensors,scores,use_sampler=False):
            ...
            backgroundlidar=inputlidar[background]
        ...
        def __getitem__(self, idx):
            ...
            for cav_id, selected_cav_base in base_data_dict.items():
                ...
                if self.sample_method == 'gt':  
                    if cav_id!=ego_id:
                        infra_lidar=selected_cav_processed['projected_lidar']
                        gt=self.gt_dict[idx]['tensor']
                        if gt!=None:

                            filtered_lidar,points_num,stat=self.box_filtering(infra_lidar,gt)
                        else:
                            filtered_lidar=infra_lidar[0:0]
                            stat=[]
                            stat=torch.tensor(stat).cuda()
                            points_num=0
                        selected_cav_processed['projected_lidar']=filtered_lidar
                elif self.sample_method == 'unc':
                    
                    if cav_id!=ego_id:
                        infra_lidar=selected_cav_processed['projected_lidar']
                        if cav_id not in cav_dict:
                            tensors=None
                        else:
                            sample_dict=self.detect_result[idx][cav_id]
